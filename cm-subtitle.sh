...
# 正規表現で「YYYY年MM月DD日」を抽出
if [[ $file_input =~ ([0-9]{4}年)([0-9]{2}月)([0-9]{2}日)(?=[0-9]{2}時$) ]]; then
    year=${BASH_REMATCH[1]}   # 例: "2024年"
    month=${BASH_REMATCH[2]}  # 例: "10月"
    day=${BASH_REMATCH[3]}    # 例: "05日"
else
    echo "ファイル名に日付が見つかりませんでした。"
    exit 1
fi

input_file="/home/pi/CmcutScp/encode-result/${year}/${month}/${day}/$file_input/join_logo_scp.csv"
...
# ファイルが保存されるディレクトリ
result_dir="/home/pi/CmcutScp/encode-result/$file_input"
...
csv_file="/home/pi/CmcutScp/encode-result/$file_input/cm_subtitle_jls.csv"
debug_file="/home/pi/CmcutScp/encode-result/$file_input/debug_file.log"
subtitle_file="/srv/dev-disk-by-uuid-A6DA96A3DA966F75/NAS/Recorded/subtitle/$file_input.jpn.srt"

echo "$subtitle_file, $file_input, "$file_input", $output_file, $input_file, $csv_file " >> "$debug_file"
