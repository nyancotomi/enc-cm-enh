...
async function runCommands() {
  ...
  try {
    ...
    if (selectedLgdFile) {
        await writeToDebugLog(debug_file, `該当する .lgd ファイル (${path.basename(selectedLgdFile)}) を使用せずにカットします。\n`);

        // join_logo_scpコマンドを実行
        const join_logo_scp = `join_logo_scp -inscp "${inscp_pas}" -incmd "/home/pi/CmcutScp/join_logo_scp_trial/JL/JL_ATX.txt" -o "/home/pi/CmcutScp/result/${file_name}/join_logo_scp.txt"`;
        await writeToDebugLog(debug_file, `cmd : join_logo_scp -inscp "${inscp_pas}" -incmd "/home/pi/CmcutScp/join_logo_scp_trial/JL/JL_ATX.txt" -o "/home/pi/CmcutScp/result/${file_name}/join_logo_scp.txt"\n`);
    ...
    // ファイルの読み込み
    fs.readFile(joinInputFile, 'utf8', async (err, data) => {
        if (err) {
            console.error('ファイルの読み込みエラー:', err);
            return;
        }
        ...
        // ファイルへの書き込み
        fs.writeFile(joinOutputFile, data, async (err) => {
            if (err) {
            ...
            if (firstColumnValue === '0') {
                const externalCommand = `/home/pi/EPGStation/config/run_encode_error.sh encod_error "${data}"`; // ここに外部コマンドを記述
                ...
                throw new Error("1列目の1行目の値が0のため、処理を失敗とします。");
            }
        });
    });
    ...
    if (isDualMono) {
        ...
        // audio dataをコピー
        Array.prototype.push.apply(args, ['-c:a', 'aac','-ac','6']);
    }
