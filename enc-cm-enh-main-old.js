...
const new_result_pas =`/home/pi/CmcutScp/encode-result/${year}/${month}/${day}/`
const joinInputFile = `${new_result_pas}/${file_name}/join_logo_scp.txt`; // 入力ファイルのパス
const debug_file = `${new_result_pas}/${file_name}/join_debug.log`;
let debug_script = '';
...
async function generateFilterComplex(joinOutputFile) {
    return new Promise(async (resolve, reject) => {
      ...
      //const frameInfoFile = `${new_result_pas}/${file_name}/temp_join_logo_scp.csv`;
      // FPS を設定
      const fps = 29.97;
...
// cm-subtitle.sh を実行する関数
async function runSubtitleScript() {
    try {
        const subtitleEnv = {
            ...process.env,
            INPUT: file_path, // 前のエンコード処理の出力ファイルを入力として指定
            OUTPUT: `/srv/dev-disk-by-uuid-A6DA96A3DA966F75/NAS/Recorded/subtitle/${file_name}.jpn.srt`,      // シェルスクリプトの出力ファイルを指定
        };

        await writeToDebugLog(debug_file, "subtitle.sh を実行します...\n");

        // シェルスクリプト cm-subtitle.sh の実行
        await execPromise(`/home/pi/EPGStation/config/subtitle.sh`, { env: subtitleEnv });
        await writeToDebugLog(debug_file, "subtitle.sh の実行が完了しました。\n");
    } catch (error) {
        await writeToDebugLog(debug_file, "字幕処理中にエラーが発生しました\n");
    }
}

// cm-subtitle-12.sh を実行する関数
async function runSubtitleScript12() {

    return new Promise((resolve, reject) => {
        const env = {
            INPUT: file_path,
            OUTPUT: `${output}_cmcut.jpn.srt`,
            ...process.env // 既存の環境変数も保持
        };

        exec('/home/pi/EPGStation/config/cm-subtitle-12.sh', { env }, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error executing cm-subtitle-12.sh: ${error}`);
                reject(error);
            } else {
                console.log(`cm-subtitle-12.sh output: ${stdout}`);
                resolve();
            }
        });
    });
...
async function runCommands() {
  ...
  try {
    ...
    // フォルダを作成
    const folder_path = `${new_result_pas}/${file_name}`;
    await mkdir(folder_path, { recursive: true });
    const avs_file = `${new_result_pas}/${file_name}/join_tsfile.avs`;
    ...
    await writeToDebugLog(debug_file, `エンコード年月日:\n--------\n${encode_date}\n----------\n`);
    await writeToDebugLog(debug_file, "AVSスクリプトファイルを生成完了\n");
    await writeToDebugLog(debug_file, `channelName:\n--------\n${file_name_lgd}\n----------\n`);

    const inscp_pas = `${new_result_pas}/${file_name}/chapter_inscp.txt`;
    const chapter_exe = `chapter_exe -v "${avs_file}" -lwi ${new_result_pas}/"${file_name}"/chapter_inscp.lwi -oa ${new_result_pas}/"${file_name}"/chapter_inscp.txt`;
    ...
    if (selectedLgdFile) {
        ...
        const logoframe = `logoframe "${avs_file}" -logo "${selectedLgdFile}" -oa "${new_result_pas}/${file_name}/logoframe_inlogo.txt"`;
        //console.log("logoframe:", logoframe);

        // logoframeコマンドを実行
        await writeToDebugLog(debug_file, `cmd : logoframe "${avs_file}" -logo "${selectedLgdFile}" -oa "${new_result_pas}/${file_name}/logoframe_inlogo.txt"\n`);
        ...
        const join_logo_scp = `join_logo_scp -inlogo "${new_result_pas}/${file_name}/logoframe_inlogo.txt" -inscp "${inscp_pas}" -incmd "/home/pi/CmcutScp/join_logo_scp_trial/JL/JL_標準.txt" -o "${new_result_pas}/${file_name}/join_logo_scp.txt"`;
        await writeToDebugLog(debug_file, `cmd : join_logo_scp -inlogo "${new_result_pas}/${file_name}/logoframe_inlogo.txt" -inscp "${inscp_pas}" -incmd "/home/pi/CmcutScp/join_logo_scp_trial/JL/JL_標準.txt" -o "${new_result_pas}/${file_name}/join_logo_scp.txt"\n`);
    ...
    //CSVに変換
    const joinOutputFile = `${new_result_pas}/${file_name}/join_logo_scp.csv`; // 出力ファイルのパス
    ...
    try {
        ...
        if (firstColumnValue === '0') {
            ...
            const externalCommand = `/home/pi/EPGStation/config/run_encode_error.sh encod_error "再エンコード開始\n${modifiedData}"`; // ここに外部コマンドを記述
                ...
                try {
                    ...
                    child.on('exit', async (code) => {
                        if (code === 0) {
                            ...
                            await execPromise(`node /home/pi/EPGStation/config/enc-cm-enh-0000.js`, { env: encEnv });
            ...
            const externalCommand = `/home/pi/EPGStation/config/run_encode_error.sh encod_error "${modifiedData}"`; // ここに外部コマンドを記述
            writeToDebugLog(debug_file, `------------------------\n1列目の1行目の値が0のため、外部コマンドを実行します: ${externalCommand}\n`);
            const { execSync } = require('child_process'); // execSyncを使用するためにchild_processモジュールをインポート
...
function processFile(file_name) {
    ...
    // 2. 条件をチェックして、[字]が含まれている場合に次の処理を実行
    if (file_name.includes("[字]")) {
        runSubtitleScript();    // 字幕処理その１
        runSubtitleScript12();  // 字幕処理その２
    }
}
